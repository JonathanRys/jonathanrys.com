...
// Google Calendar
export const generateOAuthUrl = (oauth2Client: Auth.OAuth2Client, redirectUrl?: string) => {
...
export const getNewToken = async (oauth2Client: Auth.OAuth2Client, userId: OAuth["id"], code: string) => {
  /**
  ...
  const tokenHandler = async (tokens: {}) => {
...
export const tryStoredCredentials = async (oauth2Client: Auth.OAuth2Client, userId: OAuth["id"]): Promise<boolean> => {
  /**
...
export const calendarEventToAppointment = (event: CalendarEvent): Appointment => {
  return {
...
export const addMeeting = async (userId: User["id"], meeting: {
  title: string,
  description: string,
  location: string,
  startDate: Date,
...
export const deleteMeeting = (options: {
  id: string
}) => {
  const SCOPES = 'https://www.googleapis.com/auth/calendar.events';
  return options;
